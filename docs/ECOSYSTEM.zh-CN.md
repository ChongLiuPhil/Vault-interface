...
对于四个公共框架网站，Cloudflare Pages 是首选网页交付平台，GitHub 继续作为权威源文件、版本历史和 CI 平台。当前 GitHub Pages URL 在协调迁移完成 Cloudflare staging、目标域名和跨项目链接验证以前仍然是正式入口。Vault Interface 的公共元数据接口仍然保持 provider-neutral；当前选择 Cloudflare 承载网页，并不意味着 Cloudflare 成为元数据契约的一部分。
